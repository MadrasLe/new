...
def iniciar_reconhecimento():
    ...
    print("NOTA: O DeepFace suporta bancos vetoriais como Milvus ou Pinecone, mas para")
    print("este demo no Colab, usamos armazenamento local (rápido e sem API keys).")
    ...
    try:
        while True:
            ...
            try:
                # 1. Prova de Vida (Anti-Spoofing)
                # Verifica se o rosto é real antes de tentar identificar
                ...
                is_real = False
                if len(faces) > 0:
                    # Pega o primeiro rosto detectado que tenha confiança razoável
                    # extract_faces retorna lista de dicts
                    face_obj = faces[0]
                    # Versões mais recentes do DeepFace retornam 'is_real'
                    is_real = face_obj.get("is_real", True)

                    if not is_real:
                        clear_output(wait=True)
                        print("⚠️ ALERTA: PROVA DE VIDA FALHOU (Possível foto/vídeo detectado)")
                        time.sleep(2)
                        continue
                ...
                clear_output(wait=True)
                if reconhecido:
                    print(f"✅ IDENTIFICADO: {nome_reconhecido} (Vivo: Sim)")
                else:
