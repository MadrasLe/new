...
def install_dependencies():
    ...
    packages = ["deepface", "opencv-python-headless", "tf-keras", "tensorflow"]
...
def emotion_detection_loop():
    ...
    print("🧠 Carregando modelo de emoções (DeepFace)... Isso pode demorar na primeira vez.")
    # Carrega modelo dummy para inicializar pesos
    try:
        DeepFace.analyze(np.zeros((100, 100, 3), dtype=np.uint8), actions=['emotion'], enforce_detection=False, silent=True)
    except:
        pass
    ...
    while True:
        try:
            ...
            if frame_count % 5 == 0:
                try:
                    # DeepFace espera BGR (OpenCV default) ou RGB? OpenCV lê BGR. DeepFace lida bem.
                    # enforce_detection=False para não crashar se não achar rosto
